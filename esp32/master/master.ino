...
<script>
var prevAlerts={};
function showToast(msg){var c=document.getElementById('toasts');var t=document.createElement('div');t.className='toast';t.textContent=msg;c.appendChild(t);setTimeout(function(){t.remove()},5000);}
function refresh(){fetch('/api/slaves?approved=1').then(function(r){return r.json()}).then(function(slaves){var grid=document.getElementById('grid');var stats=document.getElementById('stats');var alerts=slaves.filter(function(s){return s.alertActive}).length;var conn=slaves.filter(function(s){return s.registered}).length;stats.innerHTML='<span class="stat">'+slaves.length+' Patient'+(slaves.length!==1?'s':'')+'</span><span class="stat">'+conn+' Online</span>'+(alerts>0?'<span class="stat alert">&#9888; '+alerts+' Alert'+(alerts!==1?'s':'')+'</span>':'')+'<a href="/admin" class="nav-link">Admin Panel</a>';slaves.forEach(function(s){if(s.alertActive&&!prevAlerts[s.slaveId])showToast('ALERT: '+s.patientName+' - Bed '+s.bed+', Room '+s.room);});prevAlerts={};slaves.forEach(function(s){if(s.alertActive)prevAlerts[s.slaveId]=true;});slaves.sort(function(a,b){if(a.alertActive&&!b.alertActive)return -1;if(!a.alertActive&&b.alertActive)return 1;return a.slaveId.localeCompare(b.slaveId);});if(slaves.length===0){grid.innerHTML='<div class="empty"><h2>No Patients Yet</h2><p>Approve devices in the Admin Panel.</p></div>';return;}var html='';for(var i=0;i<slaves.length;i++){var s=slaves[i];html+='<div class="card '+(s.alertActive?'alerting':'')+'">';if(s.alertActive)html+='<div class="alert-bar"></div>';html+='<div class="card-top"><div><div class="patient-name">'+(s.patientName||'Unnamed')+'</div><div class="patient-id">'+s.slaveId+'</div></div><div class="status-icon '+(s.alertActive?'alert':s.registered?'connected':'offline')+'">'+(s.alertActive?'&#128680;':s.registered?'&#9829;':'&#128263;')+'</div></div>';if(s.alertActive)html+='<div class="alert-badge">&#9888; Alert Active</div>';html+='<div class="info-grid"><div class="info-cell"><div class="label">Bed</div><div class="value">'+(s.bed||'-')+'</div></div><div class="info-cell"><div class="label">Room</div><div class="value">'+(s.room||'-')+'</div></div></div>';html+='<div class="card-footer"><div class="conn-dot '+(s.registered?'on':'off')+'"></div>'+(s.registered?'Connected':'Offline')+'</div></div>';}grid.innerHTML=html;}).catch(function(e){console.error(e);});}
...
function doLogin(){var u=$('username').value,p=$('password').value;fetch('/api/admin/login',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({username:u,password:p})}).then(function(r){if(!r.ok){$('login-error').style.display='block';$('login-error').textContent='Invalid credentials';return;}authToken=btoa(u+':'+p);$('login-page').style.display='none';$('admin-page').style.display='block';loadDevices();}).catch(function(){$('login-error').style.display='block';$('login-error').textContent='Connection error';});}
function doLogout(){authToken='';$('admin-page').style.display='none';$('login-page').style.display='flex';$('username').value='';$('password').value='';}
function authHeaders(){return{'Content-Type':'application/json','Authorization':'Basic '+authToken}}
function loadDevices(){fetch('/api/slaves?all=1').then(function(r){return r.json()}).then(function(devs){var pending=devs.filter(function(d){return !d.approved});var approved=devs.filter(function(d){return d.approved});$('pending-count').textContent=pending.length;$('approved-count').textContent=approved.length;if(pending.length===0){$('pending-area').innerHTML='<div class="empty"><p>No pending devices. Waiting for slaves to connect...</p></div>';}else{var html='';for(var i=0;i<pending.length;i++){var d=pending[i];html+='<div class="device-card pending"><div class="device-info"><div class="device-id">'+d.slaveId+'</div><div class="device-meta">Detected - Awaiting approval</div></div><div class="actions"><span class="badge badge-pending">Pending</span><button class="btn btn-success btn-sm" onclick="openApproveModal(\''+d.slaveId+'\')">Approve</button><button class="btn btn-ghost btn-sm" onclick="rejectDevice(\''+d.slaveId+'\')">Reject</button></div></div>';}$('pending-area').innerHTML=html;}if(approved.length===0){$('approved-area').innerHTML='<div class="empty"><p>No approved devices yet.</p></div>';}else{var html2='';for(var j=0;j<approved.length;j++){var d2=approved[j];var st=d2.alertActive?'badge-alert':d2.registered?'badge-ok':'badge-off';var stT=d2.alertActive?'Alert':d2.registered?'Online':'Offline';html2+='<div class="device-card '+(d2.alertActive?'alerting':'approved')+'"><div class="device-info"><div class="device-id">'+d2.slaveId+'</div><div class="device-patient">'+(d2.patientName||'No name')+' - Bed '+(d2.bed||'-')+', Room '+(d2.room||'-')+'</div></div><div class="actions"><span class="badge '+st+'">'+stT+'</span>'+(d2.alertActive?'<button class="btn btn-danger btn-sm" onclick="clearAlert(\''+d2.slaveId+'\')">Clear</button>':'')+'<button class="btn btn-ghost btn-sm" onclick="openEditModal(\''+d2.slaveId+'\',\''+(d2.patientName||'')+'\',\''+(d2.bed||'')+'\',\''+(d2.room||'')+'\')">Edit</button><button class="btn btn-ghost btn-sm" onclick="deleteDevice(\''+d2.slaveId+'\')">Del</button></div></div>';}$('approved-area').innerHTML=html2;}fetch('/api/status').then(function(r){return r.json()}).then(function(ms){var modes={'1':'AP Mode','2':'STA Mode','3':'AP+STA Mode'};$('mode-info').innerHTML='<span class="mode-badge">&#128225; '+(modes[ms.mode]||'Not configured')+'</span>';}).catch(function(){});}).catch(function(e){console.error(e);});}
function openApproveModal(id){editingId=id;isApproving=true;$('modal-title').textContent='Approve: '+id;$('modal-submit').textContent='Approve & Save';$('m-id').value=id;$('m-name').value='';$('m-bed').value='';$('m-room').value='';$('modal').classList.add('show');}
function openEditModal(id,name,bed,room){editingId=id;isApproving=false;$('modal-title').textContent='Edit: '+id;$('modal-submit').textContent='Save';$('m-id').value=id;$('m-name').value=name;$('m-bed').value=bed;$('m-room').value=room;$('modal').classList.add('show');}
function closeModal(){$('modal').classList.remove('show');}
function submitModal(){var name=$('m-name').value.trim(),bed=$('m-bed').value.trim(),room=$('m-room').value.trim();if(!name||!bed||!room){alert('Fill all fields');return;}if(isApproving){fetch('/api/approve/'+editingId,{method:'POST',headers:authHeaders(),body:JSON.stringify({patientName:name,bed:bed,room:room})}).then(function(){closeModal();loadDevices();});}else{fetch('/api/slaves/'+editingId,{method:'PUT',headers:authHeaders(),body:JSON.stringify({patientName:name,bed:bed,room:room})}).then(function(){closeModal();loadDevices();});}}
function clearAlert(id){fetch('/api/clearAlert/'+id,{method:'POST',headers:authHeaders()}).then(function(){loadDevices();});}
function deleteDevice(id){if(!confirm('Delete '+id+'?'))return;fetch('/api/slaves/'+id,{method:'DELETE',headers:authHeaders()}).then(function(){loadDevices();});}
function rejectDevice(id){if(!confirm('Reject '+id+'?'))return;fetch('/api/slaves/'+id,{method:'DELETE',headers:authHeaders()}).then(function(){loadDevices();});}

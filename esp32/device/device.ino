...
bool connectMQTT() {
    ...
    Serial.printf("[MQTT] Connecting to %s:%d (My IP: %s)...\n", 
                  targetIP.c_str(), MQTT_BROKER_PORT, WiFi.localIP().toString().c_str());
...
void connectWiFi() {
    ...
    // Prevent interrupting ongoing connection handshake
    if (status == WL_IDLE_STATUS) return;

    static unsigned long lastTry = 0;
    unsigned long now = millis();
    if (now - lastTry < 20000) return; // 20 seconds retry interval
    lastTry = now;
    
    // Explicitly set STA mode to prevent AP/STA confusion
    if (WiFi.getMode() != WIFI_STA) WiFi.mode(WIFI_STA);

    Serial.printf("[WIFI] Attempting connection to %s (Status: %d)...\n", wifiSSID, (int)status);
    WiFi.begin(wifiSSID, wifiPass);
}
...
void setup() {
    ...
    if (MDNS.begin(deviceId.c_str())) {
        Serial.println("[MDNS] Resolver started");
    } else {
...
// ═══════════════════════════════════════════════════════════════════
void loop() {
    mqttConnected = (WiFi.status() == WL_CONNECTED && mqtt.connected());
